export function attachGameLoop(game) {
	try {
		game.loop = function () {
			...
			// Only run gameplay updates if in 'game' mode
			if (this.currentSceneMode === 'game') {
				...
				// Update menu scene (3D background)
				try { if (this.menuScene && typeof this.menuScene.update === 'function') this.menuScene.update(dt); } catch (_) { }
			}
