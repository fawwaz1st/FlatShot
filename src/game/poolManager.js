...
export function attachPoolManager(game) {
	try {
		game._initPools = function () {
			...
			// Enemy Pool
			this._enemyPool = [];
			const enemyCount = this._lowEnd ? 30 : 60;
			for (let i = 0; i < enemyCount; i++) {
				const e = new Enemy(this.scene);
				// already hidden by default in Enemy class
				this._enemyPool.push(e);
			}
		};

		game.getEnemyFromPool = function () {
			if (!this._enemyPool) return null;
			// find inactive
			const e = this._enemyPool.find(x => !x.active);
			if (e) return e;
			// optional: expand pool if full? or just limit spawn
