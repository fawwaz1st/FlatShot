...
export function attachPoolController(game){
	try{
		// init pools moved from game to controller
		game._initPools = function(){
			this._tracerPool = [];
			this._tracerMat = new THREE.LineBasicMaterial({ color: 0xfff1a8, transparent: true, opacity: 0.98 });
			const tracerCount = this._lowEnd ? 18 : 36;
			...
			this._pointsPool = [];
			this._pointsMat = new THREE.PointsMaterial({ color: 0xfff1a8, size: 0.06, transparent:true, opacity:0.98 });
			const pointsPoolCount = this._lowEnd ? 12 : 24;
			for (let i=0;i<pointsPoolCount;i++) {
				const maxCount = this._lowEnd ? 48 : 96;
				const geom = new THREE.BufferGeometry();
	...
	} catch(e){ console.error('[PoolController] attach error', e); }
} 