export class AudioFX {
	...
	ensureCtx() {
		if (!this.ctx) {
			...
			import('../game/config.js').then(m => {
				if (m.GameConfig && m.GameConfig.audio) {
					this._maxConcurrentSfx = m.GameConfig.audio.maxVoices || 12;
				}
			}).catch(() => { });
	...
	// Init dedicated white noise buffer for footsteps
	_initFootstepBuffer() {
		if (this._footstepBuffer) return;
		const sr = this.ctx.sampleRate;
		const length = sr * 1.0; // 1 second buffer
		const buf = this.ctx.createBuffer(1, length, sr);
		const data = buf.getChannelData(0);
		for (let i = 0; i < length; i++) {
			data[i] = Math.random() * 2 - 1; // Full range white noise
		}
		this._footstepBuffer = buf;
	}

	footstep({ run = false } = {}) {
		this._runAfterUnlock(() => {
			this.ensureCtx();
			if (!this._footstepBuffer) this._initFootstepBuffer();
			...
			src.buffer = this._footstepBuffer;
			// Randomize start position to vary the noise grain
			const offset = Math.random() * 0.5;

			// Filter: Lowpass 150-400Hz
			const filter = ctx.createBiquadFilter();
			filter.type = 'lowpass';
			// Variation logic: heavy/light steps
			const baseFreq = run ? 350 : 200;
			const rndFreq = baseFreq + (Math.random() * 150 - 75);
			filter.frequency.setValueAtTime(Math.max(100, rndFreq), now);
			filter.Q.value = 1.5; // Add punch

			// Envelope (Gain)
			const gain = ctx.createGain();
			gain.gain.setValueAtTime(0.001, now);

			// Attack: 0.01s (Quick impact)
			gain.gain.linearRampToValueAtTime(run ? 0.6 : 0.4, now + 0.01);

			// Decay: 0.1s (Exponential Fade)
			gain.gain.exponentialRampToValueAtTime(0.001, now + (run ? 0.12 : 0.15));
	...
	startGameBgm() {
		this._runAfterUnlock(() => {
			...
			try {
				if (!this._disableNoise) { const nbuf = this._getNoiseBuffer(); const nsrc = ctx.createBufferSource(); nsrc.buffer = nbuf; nsrc.loop = true; const nf = ctx.createBiquadFilter(); nf.type = 'lowpass'; nf.frequency.value = 1200; const ng = ctx.createGain(); ng.gain.value = 0.0; nsrc.connect(nf).connect(ng).connect(this.bgmGain); nsrc.start(); this.bgmNodes = [lead, pad, sub, lfo, lfoGain, lp, nsrc, nf, ng]; }
